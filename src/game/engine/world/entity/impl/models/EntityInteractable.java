...
import static game.engine.world.geo.Coordinates.getEntitiesOnCoordinates;
import static game.engine.ui.panels.game.PitchPanel.GRID_SIZE;
...
public abstract class EntityInteractable extends Entity {
    ...
    public void move(Coordinates coordinates) {
        getInfo().setPosition(coordinates);
        onMove(coordinates);
    }
    ...
    protected final boolean moveOrInteract(Direction d, int stepSize, boolean ignoreMapBorders) {
        ...
        Coordinates nextTopLeftCoords = Coordinates.nextCoords(getInfo().getPosition(), d, stepSize);
        ...
        List<Coordinates> nextOccupiedCoords = Coordinates.getNewCoordinatesOnDirection(getInfo().getPosition(), d, stepSize, GRID_SIZE / 3 / 2, getSize());
        ...
        GameBehavior gameBehavior = new GameBehavior() {
            ...
            public Function0<Unit> hostBehavior() {
                return () -> {
                    ...
                    new AttackEntityEventForwarder().invoke(e.toDto(), attackDamage);
