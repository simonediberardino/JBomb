...
import static game.engine.world.geo.Direction.*;
import static game.engine.world.geo.Direction.DOWN;
...
public abstract class Character extends MovingEntity {
    ...
    protected Direction imageDirection = getImageDirections().get(0);
    /**
    ...
    private void setImageDirection(Direction direction) {
        if (getImageDirections().contains(direction))
            imageDirection = direction;
        else if (imageDirection == null)
            imageDirection = getImageDirections().get(0);
    }
    ...
    protected void onSpawn() {
        super.onSpawn();
        new LocationChangedBehavior(toDto()).invoke();
        setAliveState(true);
        setHp(maxHp);
    }
    ...
    public void updateMovementDirection(Direction direction) {
        directionUpdateLogic(direction);
        directionUpdateGraphics(previousDirection, direction);
    }

    private void directionUpdateLogic(Direction direction) {
        // Update previousDirection and currDirection to reflect the new direction.
        previousDirection = currDirection;
        currDirection = direction;
    }

    private void directionUpdateGraphics(Direction previousDirection, Direction currDirection) {
        setImageDirection(currDirection);

        String[] baseIcons = getCharacterOrientedImages();
    ...
    @NotNull
    @Override
    public BufferedImage loadAndSetImage(@NotNull String imagePath) {
    ...
    protected void startDamageAnimation() {
        ...
        int iterations = (int) (INTERACTION_DELAY_MS / durationMs);
    ...
    public void handleMoveCommand(Command command, Direction oppositeDirection1, Direction oppositeDirection2) {
        ...
        List<Coordinates> oppositeBlocksCoordinates = Coordinates.getNewCoordinatesOnDirection(getInfo().getPosition(), command.commandToDirection(), PitchPanel.PIXEL_UNIT, getSize(), getSize());
